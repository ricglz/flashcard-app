...
- **Navigation**: Use `redirect()` in server components for route guards. Use `router.push()` in event handlers for user-triggered navigation. Never call `router.push()` during render — it causes React state update errors.
